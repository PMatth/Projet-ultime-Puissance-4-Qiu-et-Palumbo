...
void Connect4_AI::checkmouse(SDL_MouseButtonEvent *mouse, int &watchdog){
...
int Connect4_AI::OnExecute(){
    ...
    // Just an attempt to use the random functions of C++ : if the difficulty level is low, we set the first move by bot by choosing randomly the first column (between 0 and 6)
    
    // At higher levels, bot plays the 4th column because it's the best way to win.
        ...
        switch (Event.type) {
            ...
            case SDL_MOUSEBUTTONDOWN:
                ...
                if (Event.button.button == SDL_BUTTON_LEFT) {
                    ...
                    if (watchdog == 0) { // We want to prevent people from playing in a Full Column.
                        break;
                    }
                    ...
                    if (HorizontalWin(Grid) == true || VerticalWin(Grid) == true || ADWin(Grid) == true || DDWin(Grid) == true || IsBoardFull(Grid) == true) {
