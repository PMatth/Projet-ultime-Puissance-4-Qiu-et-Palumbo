...
public:
    ...
    // Basic Settings (same as with 2 players)
    ...
    int MakeBotPlay(Tiles tab[6][7], int depth, PlayerT player, Tiles type); // Make the Bot play
    
    int Alpha_Beta(Tiles tab[6][7],int depth, int alpha, int beta, PlayerT player); // Alpha Beta method
    
    int Maximum(int a, int b);
    
    int Minimum(int a, int b);
    
    int Eval(Tiles tab[6][7], PlayerT player); // Heuristic Evaluation
    
    int CheckThrees(Tiles tab[6][7], PlayerT player); // Check number of 3-series
    
    int CheckTwos(Tiles tab[6][7], PlayerT player); // Check number of 2-series
    
    void botplays(int depth, Tiles type); // Bot plays
    
    void SetCell(int RowID, int ColID, Tiles Type);
    void EraseCell(int RowID, int colID); // Erase cell that was set by SetCell
